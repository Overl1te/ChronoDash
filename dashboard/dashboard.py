...
class WidgetsEditor:
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            left_frame, text="Мои виджеты", font=("Segoe UI", 16, "bold")
        ).pack(pady=(10, 5))

        add_btn = ctk.CTkButton(
            left_frame, text="+ Добавить часы", command=self.add_new_clock
        )
        ...
        ctk.CTkLabel(preview_frame, text="Превью", font=("Segoe UI", 14, "bold")).pack(
        ...
        self.name_entry.bind(
            "<KeyRelease>", lambda e: self.update_cfg("name", self.name_entry.get())
        )

        # === Вкладка: Позиция и размер ===
        ...
        chk_details = ctk.CTkCheckBox(
            pos_header,
            text="Детальные настройки (координаты вручную)",
            variable=self.show_coords_var,
            command=self.toggle_coords_inputs,
        )
        ...
        ctk.CTkLabel(
            tab_position,
            text="В режиме редактирования используйте мышь.\nНажмите ESC для сохранения и выхода.",
            text_color="gray",
            font=("Segoe UI", 11),
        ).pack(pady=(0, 10))
        ...
        for key, label in [
            ...
            entry.bind(
                "<KeyRelease>",
                lambda e, k=key, w=entry: self.update_cfg(k, int(w.get() or 0)),
            )
        ...
        self.window_title_entry.bind(
            "<KeyRelease>",
            lambda e: self.update_cfg_path(
                "attach_to_window", "window_title", self.window_title_entry.get()
            ),
        )
        ...
        self.offset_x_entry.bind(
            "<KeyRelease>",
            lambda e: self.update_cfg_path(
                "attach_to_window", "offset_x", int(self.offset_x_entry.get() or 0)
            ),
        )
        ...
        self.offset_y_entry.bind(
            "<KeyRelease>",
            lambda e: self.update_cfg_path(
                "attach_to_window", "offset_y", int(self.offset_y_entry.get() or 0)
            ),
        )
        ...
        self.opacity_slider = ctk.CTkSlider(
            ...
            from_=50,
            to=255,
            command=lambda v: self.update_cfg("opacity", int(v)),
        )
        ...
        self.time_format_entry.bind(
            "<KeyRelease>",
            lambda e: self.update_cfg_path(
                "content", "format", self.time_format_entry.get()
            ),
        )
        ...
        self.color_entry.bind(
            "<KeyRelease>",
            lambda e: self.update_cfg_path("content", "color", self.color_entry.get()),
        )

        self.click_through_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(
            tab_appearance,
            text="Клик насквозь (не мешает работе)",
            variable=self.click_through_var,
            command=lambda: self.update_cfg(
                "click_through", self.click_through_var.get()
            ),
        ).pack(anchor="w", padx=20, pady=5)
        ...
        ctk.CTkCheckBox(
            ...
            command=lambda: self.update_cfg("always_on_top", self.always_top_var.get()),
        ).pack(anchor="w", padx=20, pady=5)
        ...
        ctk.CTkButton(
            btn_frame, text="Удалить", command=self.delete_current, fg_color="#d63031"
