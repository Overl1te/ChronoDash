...
class BaseDesktopWidget(QWidget):
    ...
    def set_edit_mode(self, active: bool):
        ...
        if active:
            ...
            self._apply_opacity()
            self.setCursor(Qt.ArrowCursor)  # СБРОС КУРСОРА В DEFAULT
            self.show()
    ...
    def paintEvent(self, event):
        ...
        if self.is_editing:
            # Заполняем весь rect почти невидимым цветом (alpha=1), чтобы ловить мышь везде
    ...
    def mouseMoveEvent(self, event):
        ...
        if (self.is_editing or self.is_preview) and not self.drag_pos:
            ...
            if mode in ["left", "right"]:
                ...
                cursor = Qt.SizeAllCursor  # Курсор перемещения (ЗАХВАТ)
